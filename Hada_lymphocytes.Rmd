...
TE_O <- function(d) TE_function(d, L = 75, beta = beta0)
TE_Si <- function(d) TE_function(d, L = 100, beta = beta0)
TE_Fe600 <- function(d) TE_function(d, L = 175, beta = beta0)
...
```{r}
MIXTE_function = function(r, L, Z.b, d = seq(0, 0.2, by = 0.001), beta = beta0){
  dE = function(yini, State, Pars){
    ...
    with(as.list(c(State, Pars)), {
      ...
      for(i in 1:n){
        H[i] = 6.24*30*u[i]/L[i]
        dI[i] = r[i]*(beta * 6.24/L[i] * exp(-H[i]) * (exp(H[i]) + H[i] - 1)) #Partial derivative of TE function over d. This is always 0 when d = 0 though.
      }
      dI = sum(dI)
      return(list(c(dI)))
    })
...
for (ii in 1:length(L)){lines(dose,TE_function(dose,L[ii],beta = beta0),col='green')}
#SEA
SEA=function(d){
  TE_function(d = d/3,L[1], beta = beta0)+TE_function(d/3,L[2],beta = beta0)+TE_function(d/3,L[3],beta = beta0)
}
