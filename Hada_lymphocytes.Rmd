...
NTE1_function = function(d, L, eta0, eta1, beta) {
...
new_Q_IDER_model = nlsLM(CA ~ NTE1_function(d, L, eta0, eta1, beta), data = modified_df, start = list(eta0 = 0.001, eta1 = 0.01, beta=3),
                 weights = (1/(modified_df$error)^2))

NTE2_function = function(d, L, eta0, eta1,alpha, beta) {
  Flux=6.242*d/L # rks to laz and spw. This is proposed NTE-TE model for lymphocytes
  A = 30 #The area of the cell nucleus for lymphocytes
  H=A*Flux 
  eta = eta0*L*exp(-eta1*L) # Eq. 9 in 18dev_bet
...
new_LQ_IDER_model = nlsLM(CA ~ NTE2_function(d, L, eta0, eta1,alpha, beta), data = modified_df, start = list(eta0 = 0.001, eta1 = 0.01, alpha=0.3, beta=2), 
                       weights = (1/(modified_df$error)^2))
```
...
L_NTE1 <- resid(new_Q_IDER_model) ** 2
L_NTE2 <- resid(new_LQ_IDER_model) ** 2
...
NTE1_AIC = AIC_function(RSS = WRSS_NTE1, k = 3)
NTE2_AIC = AIC_function(RSS = WRSS_NTE2)
IDER_AIC = AIC_function(RSS = WRSS_IDER)
TE_BIC = BIC_function(RSS = WRSS_TE, k = 1)
NTE1_BIC = BIC_function(RSS = WRSS_NTE1, k = 3)
NTE2_BIC = BIC_function(RSS = WRSS_NTE2)
IDER_BIC = BIC_function(RSS = WRSS_IDER)

information_critera_df = data.frame(Model = c("TE model","NTE1 model", "NTE2 model", "IDER model"), AIC = c(TE_AIC, NTE1_AIC, NTE2_AIC, IDER_AIC), BIC = c(TE_BIC, NTE1_BIC, NTE2_BIC, IDER_BIC))
arrange(information_critera_df, AIC)
```

Apparently TE did the worst, which is surprising. I think there might be issues with the original AIC and BIC functions, or maybe TE should be calculated in a different way (I already changed the k value to 1)
